...
agentshrink init --project-name "My Project"

# 2. Check readiness
agentshrink doctor

# 3. Start everything (gateway + backend + frontend)
...
client = OpenAI(
    ...
    api_key="<your-project-token>"  # shown after init
)

# 5. After collecting calls, analyse
agentshrink analyse
...
> **Windows note:** If `agentshrink` is not found, use `python -m agentshrink` instead,
> or add `%APPDATA%\Python\Python3xx\Scripts` to your PATH.

### With fine-tuning support

```bash
pip install -e ".[finetune]"    # adds torch, peft, accelerate, etc.
```

### Integrate your agent (one-word change)
...
# AFTER — AgentShrink active
from agentshrink import ShrinkLLM
llm = ShrinkLLM(output_dir=".agentshrink_output")
```

Your LangGraph nodes, tools, memory, callbacks — all unchanged.

See [docs/USER_QUICKSTART.md](docs/USER_QUICKSTART.md) for the full guide.
