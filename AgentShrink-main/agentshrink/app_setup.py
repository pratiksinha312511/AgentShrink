...
MAGIC_LINK_EXPIRY_SECONDS = 30 * 60  # 30 minutes
...
def request_magic_link(*, user_email: str, user_name: str | None = None) -> dict:
    ...
    entry = {
        ...
        "expires_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(time.time() + MAGIC_LINK_EXPIRY_SECONDS)),
    }
...
def consume_magic_link(*, token: str) -> dict:
    ...
    if expires_at:
        ...
        if time.time() > expiry_ts:
            ...
            save_project_registry(registry)
            raise ValueError("Magic link has expired. Please request a new one.")
    entry["status"] = "used"
