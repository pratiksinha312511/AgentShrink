...
class CentroidIndex:
    ...
    def route(self, prompt: str) -> RoutingDecision:
        ...
        # Check if the assigned model has been disabled in the catalog
        if config.get("is_local") and self._is_model_disabled(config["model_name"]):
            return RoutingDecision(
                cluster_id=best_cid,
                cluster_name=cluster_name,
                provider=config.get("provider", "openai"),
                model_name=self.fallback_model,
                model_display="API (model disabled)",
                confidence=best_sim,
                is_local=False,
                reason=f"cluster '{cluster_name}' model '{config['model_name']}' is disabled in catalog — falling back to API",
                nearest_cluster_name=cluster_name,
                nearest_similarity=best_sim,
                threshold=self.confidence_threshold,
            )
    ...
    def _is_model_disabled(self, model_name: str) -> bool:
        """Check if a model has been disabled in the model catalog."""
        try:
            from agentshrink.model_catalog import load_model_catalog
            catalog = load_model_catalog(self._output_dir)
            for model in catalog.get("models", []):
                if model.get("model_name") == model_name:
                    return not model.get("enabled", True)
            return False  # Model not in catalog = not disabled
        except Exception:
            return False  # If catalog can't be loaded, don't block routing
