...
CREATE INDEX IF NOT EXISTS idx_audit_ts ON routing_audit_log(timestamp);
CREATE INDEX IF NOT EXISTS idx_audit_action ON routing_audit_log(action);
"""


def _ensure_audit_table():
    """Create the routing audit log table if it doesn't exist."""
    if not DB_PATH.exists():
        DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    with sqlite3.connect(DB_PATH) as conn:
        conn.executescript(ROUTING_AUDIT_TABLE_SQL)
...
def _write_audit_entry(
    ...
    *,
    ...
    """Persist a routing config change to the audit log."""
    _ensure_audit_table()
    with sqlite3.connect(DB_PATH) as conn:
        conn.execute(
            """INSERT INTO routing_audit_log
               (timestamp, action, user_email, cluster_count, detail, backup_path)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (
...
def _validate_model_access(*, backend: str, model_id: str, hf_token: str | None) -> None:
    ...
    if needs_hf:
        try:
            ...
            # Allow SSL errors (corporate proxy) to pass through — the actual
            # training backend will surface a clear error if access is denied.
            exc_str = str(exc)
            if "CERTIFICATE_VERIFY_FAILED" in exc_str or "SSL" in exc_str:
                import logging as _logging
                _logging.getLogger(__name__).warning(
                    "Skipping HF access validation for %s due to SSL error (corporate proxy). "
                    "Training will proceed and fail later if the token is actually invalid.",
                    model_id,
                )
            else:
                raise HTTPException(
...
app.add_middleware(
    ...
    allow_headers=["Authorization", "Content-Type", "X-AgentShrink-Node", "X-AgentShrink-Run-Id"],
)
...
async def get_status():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        active_provider = _active_provider_context()
        baseline_model = active_provider["gateway_model"] or os.getenv("AGENTSHRINK_ESTIMATE_MODEL") or "gpt-4o-mini"
...
async def apply_report():
    ...
    backup_path = ""
    if existing:
        backup_path = str(_save_routing_backup(existing, reason="preapply"))
    ...
    _write_audit_entry(
        ...
        detail={"clusters": list(config_payload.get("routing", {}).keys())},
...
async def rollback_routing_config():
    ...
    backup_path = ""
    if current:
        backup_path = str(_save_routing_backup(current, reason="prerollback"))
    ...
    _write_audit_entry(
        "rollback",
        cluster_count=cluster_count,
        detail={"restored_from": backup.name},
        backup_path=backup_path,
    )
...
async def get_routing_audit_log(
    ...
    """
    GET /api/routing/audit-log
    Paginated, filterable audit log of routing config changes.
    Query params:
      limit   — max rows (1–500, default 50)
      offset  — pagination offset
      action  — filter by action type (apply, rollback)
      since   — ISO timestamp lower bound
      until   — ISO timestamp upper bound
      fmt     — "csv" to return CSV download
    """
    _ensure_audit_table()
    ...
    with sqlite3.connect(DB_PATH) as conn:
        conn.row_factory = sqlite3.Row
        total = conn.execute(f"SELECT COUNT(*) FROM routing_audit_log {where}", params).fetchone()[0]
        rows = conn.execute(
            f"SELECT * FROM routing_audit_log {where} ORDER BY id DESC LIMIT ? OFFSET ?",
            params + [limit, offset],
        ).fetchall()
    ...
    if fmt == "csv":
        ...
        import io
        buf = io.StringIO()
        writer = csv.DictWriter(buf, fieldnames=["id", "timestamp", "action", "user_email", "cluster_count", "detail", "backup_path"])
        writer.writeheader()
        for e in entries:
        ...
        return Response(
            ...
            media_type="text/csv",
            headers={"Content-Disposition": "attachment; filename=routing_audit_log.csv"},
        )
...
async def product_test_gateway():
    ...
    try:
        ...
        detail = "Gateway returned an error response."
        raise HTTPException(status_code=400, detail=f"Gateway test failed: {detail}") from exc
    except Exception as exc:
        raise HTTPException(status_code=400, detail="Gateway test failed: could not connect to gateway.") from exc
