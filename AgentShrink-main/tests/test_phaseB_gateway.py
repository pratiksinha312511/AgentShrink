...
_TEST_TOKEN = "as_live_test_phaseB"
...
def test_gateway_chat_completion_passthrough_and_logging():
    ...
    client = TestClient(create_gateway_app(upstream=upstream, db_path=db, expected_api_key=_TEST_TOKEN))
    response = client.post(
        "/v1/chat/completions",
        headers={**_AUTH_HEADER, "x-agentshrink-node": "gateway_test_node", "x-agentshrink-run-id": "gateway-run"},
        json={
            "model": "moonshotai/kimi-k2-instruct",
            "messages": [
...
def test_gateway_streaming_passthrough_and_logging():
    db = _temp_db()
    client = TestClient(create_gateway_app(upstream=_FakeUpstream(), db_path=db, expected_api_key=_TEST_TOKEN))
    with client.stream(
        "POST",
        "/v1/chat/completions",
...
def test_gateway_metadata_overrides_header_node_name():
    ...
    response = client.post(
        "/v1/chat/completions",
        headers={**_AUTH_HEADER, "x-agentshrink-node": "header_node", "x-agentshrink-run-id": "run-meta"},
        json={
            "model": "moonshotai/kimi-k2-instruct",
...
def test_gateway_multiple_calls_keep_separate_run_ids():
    ...
    first = client.post(
        "/v1/chat/completions",
        headers={**_AUTH_HEADER, "x-agentshrink-node": "node_a", "x-agentshrink-run-id": "run-a"},
        json={"model": "moonshotai/kimi-k2-instruct", "messages": [{"role": "user", "content": "first"}]},
    )
    second = client.post(
        "/v1/chat/completions",
        headers={**_AUTH_HEADER, "x-agentshrink-node": "node_b", "x-agentshrink-run-id": "run-b"},
...
def test_gateway_routes_to_local_when_cluster_is_local():
    ...
    client = TestClient(create_gateway_app(upstream=_FakeUpstream(), db_path=db, router=_FakeRouter(decision), local_executor=local, expected_api_key=_TEST_TOKEN))
    response = client.post(
        "/v1/chat/completions",
        headers=_AUTH_HEADER,
...
def test_gateway_uses_upstream_for_unmatched_route():
    ...
    client = TestClient(create_gateway_app(upstream=_FakeUpstream(), db_path=db, router=_FakeRouter(decision), local_executor=_FakeLocalExecutor(), expected_api_key=_TEST_TOKEN))
    response = client.post(
        "/v1/chat/completions",
        headers=_AUTH_HEADER,
...
def test_mock_upstream_runs_without_network():
    db = _temp_db()
    client = TestClient(create_gateway_app(upstream=OpenAICompatibleUpstream(provider="mock"), db_path=db, expected_api_key=_TEST_TOKEN))
    response = client.post(
        "/v1/chat/completions",
        headers=_AUTH_HEADER,
        json={
